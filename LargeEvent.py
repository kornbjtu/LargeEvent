...
class Depot(sim.Component, AbstractDepot):
    def setup(self, id, node, max_order, capacity, serve_time_dist, serve_queue, max_wait_time, order_list, truck_list) -> None:
        AbstractDepot.__init__(self, id, node, max_order, capacity, serve_time_dist, serve_queue, max_wait_time, order_list, truck_list)
        self.service_center = ServiceCenter(capacity=capacity, serve_time_dist=serve_time_dist, serve_queue=serve_queue, depot=self)
    ...
    def process(self):
        ...
        while True:
            ...
            if (self.__get_time_wait() >= self.max_wait_time) or (sum_order_volume >= self.max_order): 
                ...
                self.order_list = [item for item in self.order_list if item in clear_order_list]
...
class Truck(sim.Component, AbstractTruck):
    ...
    def __decide_return_depot(self) -> Depot:
        ...
        instock_depots = [len(depot.get_truck_instock()) for depot in self.depot_list]
...
class OrderGen(sim.Component):
    ...
    def process(self):
        while True:
            # 动态调整目的地分布
            self.dest_dist = self.adjust_dest_distribution(self.original_dest_dist, self.event_gen)
...
if __name__ == '__main__':
    ...
    ############## UTENTILS #######################
    h2s = lambda x: x * 60 * 60  # convert hours to seconds

    m2s = lambda x: x * 60 # convert minutes to seconds
    ...
    # 一天按12小时算总共43200s 
    VENUES = [
        Venue(node=map.node(1), start_time = h2s(3), duration =  DURATION_DIST.sample(), event_scale=2, influence_road=AFFECT_ROAD[1],affected_node=ORDER_AFFECT_NODE[1]),
        Venue(node=map.node(2), start_time = h2s(6), duration =  DURATION_DIST.sample(), event_scale=3, influence_road=AFFECT_ROAD[2],affected_node=ORDER_AFFECT_NODE[2]),
        Venue(node=map.node(3), start_time = h2s(9), duration =  DURATION_DIST.sample(), event_scale=1, influence_road=AFFECT_ROAD[3],affected_node=ORDER_AFFECT_NODE[3]),
        Venue(node=map.node(4), start_time = h2s(6), duration =  DURATION_DIST.sample(), event_scale=4, influence_road=AFFECT_ROAD[4],affected_node=ORDER_AFFECT_NODE[4]),
        Venue(node=map.node(5), start_time = h2s(6.5), duration = DURATION_DIST.sample(), event_scale=5, influence_road=AFFECT_ROAD[5],affected_node=ORDER_AFFECT_NODE[5])
    ]
    ...
    NUM_TRUCK = 5 * len(DEPOT_LIST) ## we assume each depot has exactly only 5 vehicles
    ...
    for ind in range(NUM_TRUCK):
        assigned_depot = depot_list[ind % NUM_DEPOT]  # Assign depot in a round-robin manner
        truck_list.append(Truck(id=ind, order_list=[], act_time=None, depot=assigned_depot, depot_list=depot_list))
