    ...
    def animate_venues(self):
        for venue in self.all_venues:
            ...
            if venue.cong_level==0:
                ...
                cv2.circle(self.canvas, self.trans(venue.node.x, venue.node.y), self.venue_radius, self.venue_color[5], -1)
                cv2.putText(self.canvas, '5', self.trans(venue.node.x-5, venue.node.y-5), self.font, 0.5, (255, 255, 255), 1)
            else:
        ...
        for road in self.map.roads:
            if road.road_type == False:
                ...
                # normal road and no congestion
                    ...
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[0], self.road_nor_width)
                elif road.cong == 1:
                    # normal road with congestion rank1
                    
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[1], self.road_nor_width)
                elif road.cong == 2:
                    # normal road with congestion rank2
                    
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[2], self.road_nor_width)
                elif road.cong == 3:
                    # normal road with congestion rank3
                    
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[3], self.road_nor_width)
                elif road.cong == 4:
                    # normal road with congestion rank4
                    
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[4], self.road_nor_width)
                elif road.cong == 5:
                    # normal road with congestion rank5
                    
                    cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[5], self.road_nor_width)
                else:
                    print('Road_color error!!!')
            else:
                # highways
                cv2.line(self.canvas, self.trans(road.node1.x, road.node1.y), self.trans(road.node2.x, road.node2.y), self.road_color[0], self.road_high_width)
