...
from tkinter import filedialog, IntVar, BooleanVar, StringVar, DoubleVar
import json
...
def get_simulation_params():
    ...
    def load_defaults():
        ...
        energy_per_sec.set(defaults["Simulation & KPI-related Settings"]["Energy consumption per second"])
        energy_per_meter.set(defaults["Simulation & KPI-related Settings"]["Energy consumption per meter"])
    ...
    batch_run_label.grid(row=13, column=0, sticky='w')
    run_times = tk.IntVar()
    ...
    tk.Label(root, text="Number of Ritsch Truck:", anchor="w").grid(row=17, column=0, sticky='w')
    ritsch_trucks = tk.IntVar()
    tk.Entry(root, textvariable=ritsch_trucks).grid(row=17, column=1, columnspan=2, sticky='w')

    tk.Label(root, text="Number of Dongfeng Truck:", anchor="w").grid(row=18, column=0, sticky='w')
    dongfeng_trucks = tk.IntVar()
    tk.Entry(root, textvariable=dongfeng_trucks).grid(row=18, column=1, columnspan=2, sticky='w')

    tk.Label(root, text="Number of Isuzu Truck:", anchor="w").grid(row=19, column=0, sticky='w')
    isuzu_trucks = tk.IntVar()
    tk.Entry(root, textvariable=isuzu_trucks).grid(row=19, column=1, columnspan=2, sticky='w')

    tk.Label(root, text="Number of Volvo Truck:", anchor="w").grid(row=20, column=0, sticky='w')
    volvo_trucks = tk.IntVar()
    tk.Entry(root, textvariable=volvo_trucks).grid(row=20, column=1, columnspan=2, sticky='w')
    ...
    tk.Label(root, text="Start delivery threshold (volume):", anchor="w").grid(row=22, column=0, sticky='w')
    ...
    tk.Label(root, text="Speed on Motorway (km/h):", anchor="w").grid(row=24, column=0, sticky='w')
    speed_motorway = tk.DoubleVar()
    tk.Entry(root, textvariable=speed_motorway).grid(row=24, column=1, columnspan=2, sticky='w')

    tk.Label(root, text="Speed on City Street (km/h):", anchor="w").grid(row=25, column=0, sticky='w')
    speed_city_street = tk.DoubleVar()
    tk.Entry(root, textvariable=speed_city_street).grid(row=25, column=1, columnspan=2, sticky='w')

    tk.Label(root, text="Truck parameters file:", anchor="w").grid(row=26, column=0, sticky='w')
    truck_params_file = StringVar()
    tk.Entry(root, textvariable=truck_params_file).grid(row=26, column=1, sticky='w')
    tk.Button(root, text="Browse", command=lambda: truck_params_file.set(filedialog.askopenfilename())).grid(row=26, column=2)
    ...
    button_frame.grid(row=29, column=0, columnspan=3)
